...
static RT64::Application* live_rt64_application = nullptr;
...
void renderer::apply_extra_graphics_options() {
    if (live_rt64_application == nullptr) {
        return;
    }
    ...
    try {
        ...
        // Graphics tab has not been created yet.
    }
}

static bool turok_host_key_reserved_for_filter(SDL_Scancode scancode, bool inspector_open) {
    switch (scancode) {
    ...
    case SDL_SCANCODE_ESCAPE:
        return true;
    case SDL_SCANCODE_F3: // Host debug HUD unless the inspector is eating keys.
...
bool renderer::is_rt64_inspector_open() {
    if (live_rt64_application == nullptr || live_rt64_application->presentQueue == nullptr) {
        return false;
    }
    const std::lock_guard lock(live_rt64_application->presentQueue->inspectorMutex);
    return live_rt64_application->presentQueue->inspector != nullptr;
}

bool renderer::toggle_rt64_inspector() {
    if (live_rt64_application == nullptr) {
        std::fprintf(stderr, "[rt64] inspector: renderer not ready\n");
        return false;
    }
    const auto api = live_rt64_application->chosenGraphicsAPI;
    if (api != RT64::UserConfiguration::GraphicsAPI::D3D12 &&
        api != RT64::UserConfiguration::GraphicsAPI::Vulkan) {
        // Inspector ctor/newFrame only init D3D12 and Vulkan. Metal hits the
        // default branch (assert stripped in this build) and ImGui::NewFrame
        // then SIGSEGVs on the gfx thread.
        std::fprintf(stderr,
            "[rt64] inspector skipped: API is not D3D12/Vulkan (this Mac build uses Metal). "
            "F2 dumps textures without the editor.\n");
        return false;
    }
    live_rt64_application->userConfig.developerMode = true;
    live_rt64_application->processDeveloperShortcut(
        RT64::Application::DeveloperShortcut::Inspector);
    const bool open = is_rt64_inspector_open();
    std::fprintf(stderr, "[rt64] inspector %s (F1)\n", open ? "open" : "closed");
    return open;
}

bool renderer::is_dumping_textures() {
    return live_rt64_application != nullptr &&
           live_rt64_application->state != nullptr &&
           !live_rt64_application->state->dumpingTexturesDirectory.empty();
}

bool renderer::toggle_texture_dump() {
    if (live_rt64_application == nullptr || live_rt64_application->state == nullptr) {
        std::fprintf(stderr, "[rt64] texture dump: renderer not ready\n");
        return false;
    }

    auto& directory = live_rt64_application->state->dumpingTexturesDirectory;
    if (!directory.empty()) {
        directory.clear();
        std::fprintf(stderr, "[rt64] texture dump stopped\n");
        return false;
    }

    const std::filesystem::path dump_dir = recomp::get_config_path() / "texture-dumps";
    std::error_code ec;
    std::filesystem::create_directories(dump_dir, ec);
    if (ec) {
        std::fprintf(stderr, "[rt64] texture dump: failed to create %s (%s)\n",
                     dump_dir.string().c_str(), ec.message().c_str());
        return false;
    }

    live_rt64_application->state->textureManager.dumpedSet.clear();
    directory = dump_dir;
    std::fprintf(stderr, "[rt64] texture dump started: %s\n", dump_dir.string().c_str());
    return true;
}

bool renderer::filter_sdl_event(void* sdl_event) {
    if (live_rt64_application == nullptr || sdl_event == nullptr) {
        return false;
    }
    auto* event = static_cast<SDL_Event*>(sdl_event);
    const bool inspector_open = is_rt64_inspector_open();
    if (event->type == SDL_KEYDOWN &&
        turok_host_key_reserved_for_filter(event->key.keysym.scancode, inspector_open)) {
        return false;
    }
    return live_rt64_application->sdlEventFilter(event);
}
...
renderer::RT64Context::RT64Context(uint8_t* rdram, ultramodern::renderer::WindowHandle window_handle, ultramodern::renderer::PresentationMode presentation_mode, bool debug) {
    ...
    if (setup_result != ultramodern::renderer::SetupResult::Success) {
        app = nullptr;
        live_rt64_application = nullptr;
        return;
    }

    live_rt64_application = app.get();
...
renderer::RT64Context::~RT64Context() {
    if (live_rt64_application == app.get()) {
        live_rt64_application = nullptr;
    }
}
...
void renderer::RT64Context::send_dl(const OSTask* task) {
    ...
    app->state->rsp->reset();
    // Unique DLs: one authored display list per host VI. Do not read
    // gNextTicks as a 30/15 source and do not ask RT64 to interpolate.
    // TUROK2_UNIQUE_120 advertises source=120 so timestamps match the
    // extra DLs. targetRate stays 60 here; workload_queue still forces 0
    // unless TUROK2_RT64_INTERPOLATE is set. That is not 60→120 matching.
    constexpr uint32_t NextTicksRdramOffset = 0x0011AD10;
    int32_t nextTicks = 0;
    std::memcpy(&nextTicks, app->core.RDRAM + NextTicksRdramOffset,
                sizeof(nextTicks));
    uint32_t display = ultramodern::get_display_refresh_rate();
    if (display == 0) {
        display = 60;
    }
    ...
    if (std::getenv("TUROK2_FPS_DIAGNOSTICS") != nullptr) {
        ...
        if (nextTicks != lastTicks || display != lastDisplay || source_hz != lastSource) {
            std::fprintf(stderr,
                         "[rt64:source-rate] ticks=%d source=%uHz present=%uHz display=%u interp=off\n",
                         nextTicks, source_hz, source_hz, display);
            lastTicks = nextTicks;
            lastDisplay = display;
            lastSource = source_hz;
        }
    }
