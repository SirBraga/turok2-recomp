...
namespace recompui {
    namespace config {
        ...
        bool graphics::get_developer_mode() {
            return get_graphics_bool_value(graphics::options::developer_mode);
        }
        ...
        double graphics::get_fov_scale() {
            return get_graphics_number_value<double>(graphics::options::fov_scale);
        }
        ...
        recomp::config::Config &create_graphics_tab(const std::string &name) {
            ...
            config.add_bool_option(
                ...
                "Dev Mode",
                "Enables the RT64 editor. F1 toggles it even if this is off. F2 dumps texture hashes to runtime-data/texture-dumps (no file dialog). Do not commit those dumps.",
                false,
                true
            );
