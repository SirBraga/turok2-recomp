...
extern "C" void osSpTaskStartGo_recomp(uint8_t* rdram, recomp_context* ctx) {
    // TLB-mapped games may hand libultra an RDRAM physical address here.
    // Convert that low address to its KSEG0 alias before TO_PTR subtracts the
    // KSEG base. Turok 2 uses task addresses such as 0x00000030.
    uint32_t task_addr = static_cast<uint32_t>(ctx->r4);
    if (task_addr < 0x00800000U) {
        task_addr = RDRAM_OFFSET(0, task_addr) | 0x80000000U;
    }

    OSTask* task = TO_PTR(OSTask, static_cast<PTR(OSTask)>(task_addr));
    ...
    ultramodern::submit_rsp_task(rdram, static_cast<PTR(OSTask)>(task_addr));
}
