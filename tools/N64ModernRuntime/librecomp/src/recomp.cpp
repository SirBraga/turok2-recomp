...
extern "C" void do_break(uint32_t vram) {
    // Report on stderr and flush before leaving. This used to print to stdout,
    // which is block buffered when redirected to a file, and the exit below tears
    // the process down from whichever game thread hit the break: if that teardown
    // crashes, the buffer never reaches the file and the break looks like it never
    // happened. The message is the only thing that names where the trap was, so it
    // has to survive.
    fprintf(stderr, "Encountered break at original vram 0x%08X\n", vram);
    fflush(stderr);
    fflush(stdout);
    assert(false);
    exit(EXIT_FAILURE);
