...
// Turok 2 pages audio banks into useg 0x00400000-0x00900000, which the
// runtime relocates to 0x00B00000. Hardware RSP DMA is physical: after
// osVirtualToPhysical those banks become 0x00Bxxxxx. Forcing KSEG0 on that
// physical hits the 8 MiB mask and reads heap instead of the bank.
static inline int64_t rsp_guest_dram_addr(uint32_t dram_addr) {
    const uint32_t phys = dram_addr & 0x00FFFFF8u;
    if (phys >= RECOMP_USEG_BANKS_BASE && phys < 0x01000000u) {
        return (int64_t)(uint32_t)(RECOMP_USEG_BANKS_START + (phys - RECOMP_USEG_BANKS_BASE));
    }
    // Untranslated useg still appears in some ABI words. Heap AI buffers
    // live at physical 0x007xxxxx after KSEG0 strip, so stop before that.
    if (phys >= RECOMP_USEG_BANKS_START && phys < 0x00700000u) {
        return (int64_t)(uint32_t)phys;
    }
    return (int64_t)(int32_t)(phys + 0x80000000u);
}
...
static inline void dma_rdram_to_dmem(uint8_t* rdram, uint32_t dmem_addr, uint32_t dram_addr, uint32_t rd_len) {
    ...
    const int64_t guest = rsp_guest_dram_addr(dram_addr);
    assert(dmem_addr + rd_len <= 0x1000);
    for (uint32_t i = 0; i < rd_len; i++) {
        RSP_MEM_B(i, dmem_addr) = MEM_B(0, guest + i);
    }
}
...
static inline void dma_dmem_to_rdram(uint8_t* rdram, uint32_t dmem_addr, uint32_t dram_addr, uint32_t wr_len) {
    ...
    for (uint32_t i = 0; i < wr_len; i++) {
        MEM_B(0, guest + i) = RSP_MEM_B(i, dmem_addr);
    }
}
