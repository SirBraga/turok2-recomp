...
extern "C" void osCreateThread(RDRAM_ARG PTR(OSThread) t_, OSId id, PTR(thread_func_t) entrypoint, PTR(void) arg, PTR(void) sp, OSPri pri) {
    ...
    // TEMPORARY DIAGNOSTIC: record every thread's stack top and report when a new
    // one starts close to an existing one.
    //
    // Turok 2 loses a callee saved register out of a live stack slot with the
    // stack pointer unchanged and no instrumented call responsible, which points
    // at a concurrent writer. Two threads whose stacks overlap would produce
    // exactly that, and stack tops are the cheapest place to see it. Stacks grow
    // downwards, so two tops closer together than the lower one's frame usage
    // means they share memory.
    {
        struct StackRecord {
            OSId id;
            uint32_t entry;
        ...
        static StackRecord records[24];
        static uint32_t count = 0;

        const uint32_t top = (uint32_t)sp;
        for (uint32_t i = 0; i < count; i++) {
            const uint32_t other = records[i].top;
            const uint32_t distance = (top > other) ? (top - other) : (other - top);
            if (distance < 0x1000) {
                fprintf(stderr,
                    "[thread:stack] thread %d entry %08X top %08X is %X bytes from thread %d entry %08X top %08X\n",
                    id, (uint32_t)entrypoint, top,
                    distance, records[i].id, records[i].entry, other);
                fflush(stderr);
            }
        }
        if (count < std::size(records)) {
            records[count++] = { id, (uint32_t)entrypoint, top };
        }
