...
bool do_send(RDRAM_ARG PTR(OSMesgQueue) mq_, OSMesg msg, bool jam, bool block) {
    OSMesgQueue* mq = TO_PTR(OSMesgQueue, mq_);
    const uint32_t trace_queue_addr = static_cast<uint32_t>(mq_);
    const bool trace_turok_main_queue = std::getenv("TUROK2_RUNTIME_DIAGNOSTICS") != nullptr &&
        (trace_queue_addr == 0x8011AD28u || trace_queue_addr == 0x8011ADD0u ||
         trace_queue_addr == 0x8011AE08u);
    static uint32_t traced_main_sends = 0;
    if (trace_turok_main_queue && traced_main_sends < 128) {
        fprintf(stderr, "[mq:send-before] q=%08X valid=%d count=%d first=%d recv_head=%08X\n",
            static_cast<uint32_t>(mq_), mq->validCount, mq->msgCount, mq->first,
    ...
    if (!ultramodern::thread_queue_empty(PASS_RDRAM blocked_queue)) {
        PTR(OSThread) resumed = ultramodern::thread_queue_pop(PASS_RDRAM blocked_queue);
        if (std::getenv("TUROK2_RUNTIME_DIAGNOSTICS") != nullptr) {
            fprintf(stderr,
                "[mq:wake] q=%08X msg=%08X valid=%d count=%d recv_head_after=%08X resumed=%08X external=%d\n",
                static_cast<uint32_t>(mq_), static_cast<uint32_t>(msg),
                mq->validCount, mq->msgCount,
                static_cast<uint32_t>(mq->blocked_on_recv),
                static_cast<uint32_t>(resumed), block ? 0 : 1);
            fflush(stderr);
        }
        if (trace_turok_main_queue && traced_main_sends < 128) {
            fprintf(stderr, "[mq:send-wake] q=%08X thread=%08X\n",
                static_cast<uint32_t>(mq_), static_cast<uint32_t>(resumed));
        }
        ultramodern::schedule_running_thread(PASS_RDRAM resumed);
    }
    if (trace_turok_main_queue && traced_main_sends++ < 128) {
        fprintf(stderr, "[mq:send-after] q=%08X valid=%d recv_head=%08X\n",
            static_cast<uint32_t>(mq_), mq->validCount,
            static_cast<uint32_t>(mq->blocked_on_recv));
    }
...
bool do_recv(RDRAM_ARG PTR(OSMesgQueue) mq_, PTR(OSMesg) msg_, bool block) {
    ...
    const bool trace_turok_main_queue = std::getenv("TUROK2_RUNTIME_DIAGNOSTICS") != nullptr &&
        (trace_queue_addr == 0x8011AD28u || trace_queue_addr == 0x8011ADD0u ||
         trace_queue_addr == 0x8011AE08u);
    static uint32_t traced_main_recvs = 0;
    if (trace_turok_main_queue && traced_main_recvs < 128) {
        fprintf(stderr, "[mq:recv-before] q=%08X valid=%d count=%d first=%d block=%d thread=%08X\n",
            static_cast<uint32_t>(mq_), mq->validCount, mq->msgCount, mq->first,
            block ? 1 : 0, static_cast<uint32_t>(ultramodern::this_thread()));
    }
    if (!block) {
    ...
    if (trace_turok_main_queue && traced_main_recvs++ < 128) {
        fprintf(stderr, "[mq:recv-after] q=%08X valid=%d msg=%08X\n",
            static_cast<uint32_t>(mq_), mq->validCount,
            msg_ != NULLPTR ? static_cast<uint32_t>(*TO_PTR(OSMesg, msg_)) : 0u);
    }
