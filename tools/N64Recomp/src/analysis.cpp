...
bool N64Recomp::analyze_function(const N64Recomp::Context& context, const N64Recomp::Function& func,
    ...
    for (size_t i = 0; i < stats.jump_tables.size(); i++) {
        ...
        // Jump tables are commonly emitted in a read-only data section rather
        // than alongside the function that uses them. Resolve the table's
        // section by its VRAM so that split code/rodata ELFs use the correct
        // ROM offset.
        const Section* jtbl_section = nullptr;
        for (size_t section_index = 0; section_index < context.sections.size(); section_index++) {
            const Section& candidate = context.sections[section_index];
            if (cur_jtbl.vram >= candidate.ram_addr &&
                cur_jtbl.vram - candidate.ram_addr < candidate.size) {
                jtbl_section = &candidate;
                cur_jtbl.section_index = static_cast<uint16_t>(section_index);
                break;
            }
        }

        if (jtbl_section == nullptr || jtbl_section->rom_addr == static_cast<uint32_t>(-1)) {
            fmt::print("Failed to find section for jump table at 0x{:08X} used by instruction at 0x{:08X}\n",
                cur_jtbl.vram, cur_jtbl.jr_vram);
            return false;
        }

        cur_jtbl.rom = jtbl_section->rom_addr + cur_jtbl.vram - jtbl_section->ram_addr;

        while (vram < end_address) {
            // Retrieve the current entry of the jump table
            uint32_t rom_addr = cur_jtbl.rom + vram - cur_jtbl.vram;
