...
uint32_t expected_c0_reg_value(int cop0_reg) {
    ...
    case Cop0Reg::RSP_COP0_DPC_TMEM:
        // Some stock aspMain builds share the SDK's DPC bookkeeping helper.
        // RSPRecomp already models DPC status as idle; the companion counters
        // must therefore read as zero as well.
        return 0;
    default:
...
std::string_view c0_reg_write_action(int cop0_reg) {
    ...
    case Cop0Reg::RSP_COP0_DPC_END:
    case Cop0Reg::RSP_COP0_DPC_STATUS:
        // Keep DPC bookkeeping consistent with the idle/zero read model above.
        // Audio command lists do not rely on the RDP side effects of this
        // shared SDK helper.
        return "";
...
BranchTargets get_branch_targets(const std::vector<rabbitizer::InstructionRsp>& instrs) {
    ...
    for (const auto& instr : instrs) {
        if (instr.isJumpWithAddress() || instr.isBranch()) {
            // RSP execution always occurs in IMEM. Rabbitizer reports branch
            // and jump targets as offsets within the 4 KiB bank, so restore
            // the IMEM-select bit just as the hardware does.
            uint32_t target = (instr.getBranchVramGeneric() & rsp_mem_mask) | 0x1000;
            ret.direct_targets.insert(target);
        }
        if (instr.doesLink()) {
...
bool process_instruction(size_t instr_index, const std::vector<rabbitizer::InstructionRsp>& instructions, std::ofstream& output_file, const BranchTargets& branch_targets, const std::unordered_set<uint32_t>& unsupported_instructions, const ResumeTargets& resume_targets, bool has_overlays, bool indent, bool in_delay_slot) {
    ...
    uint16_t branch_target = (instr.getBranchVramGeneric() & rsp_mem_mask) | 0x1000;
    ...
    // Otherwise, implement the instruction directly
        ...
        case InstrId::rsp_bgezal:
            print_line("{}31 = 0x{:04X}", ctx_gpr_prefix(31), instr_vram + 2 * instr_size);
            print_indent();
            print_branch_condition("if (RSP_SIGNED({}{}) >= 0)", ctx_gpr_prefix(rs), rs);
            print_branch("goto L_{:04X}", branch_target);
            break;
...
bool read_config(const std::filesystem::path& config_path, RSPRecompilerConfig& out) {
    ...
    try {
        ...
        ret.entry_address = config_data["entry_address"].value_or(static_cast<uint32_t>(ret.text_address));
...
void create_function(const std::string& function_name, std::ofstream& output_file, const std::vector<rabbitizer::InstructionRsp>& instrs, const RSPRecompilerConfig& config, const ResumeTargets& resume_targets, bool is_permutation, bool is_initial) {
    ...
    branch_targets.direct_targets.insert(config.entry_address & rsp_mem_mask);
    ...
    if ((!is_permutation || is_initial) &&
        ((config.entry_address & rsp_mem_mask) != (config.text_address & rsp_mem_mask))) {
        fmt::print(output_file, "    goto L_{:04X};\n", config.entry_address & rsp_mem_mask);
    }
