...
namespace RT64 {
    ...
    // Acclaim's build of F3DEX2.NoN.fifo 2.05, used by Turok 2. It was the only
    // F3DEX2.NoN.fifo variant left as Unknown, which meant only the RDP command
    ...
    // Acclaim's build of the L3DEX2 companion that Turok 2 ships next to its
    // F3DEX2.NoN.fifo 2.05 graphics ucode. Leaving it as Unknown installs only
    // the RDP command subset, so every display list command in a task that
    // loads this ucode goes unhandled. Both stock siblings (2.05 and 2.08) use
    // L3DEX2, and this entry's hash lengths (0x1190 text / 0x3F0 data) sit in
    // the L3DEX2 buckets rather than the L3DEX 1.xx ones (0xFF0 / 0x800).
    const GBIInstance   L3DEX2_FIFO_ACCLAIM       = { "L3DEX2.fifo 2.05 (Acclaim)",                GBIUCode::L3DEX2,      { false,  false,  false,  false,  false } }; // Needs confirmation.
    ...
    GBI *GBIManager::getGBIForUCode(uint8_t *RDRAM, uint32_t textAddress, uint32_t dataAddress) {
        ...
        // TEMPORARY DIAGNOSTIC: report which database entry matched. An entry
        // whose ucode is Unknown only gets the RDP command subset installed,
        // because the switch above has no case for it and its assert is compiled
        // out in optimized builds. The display list commands are then left
        // unhandled, which produces nonsensical tile parameters downstream.
        fprintf(stderr, "[rt64:gbi] matched \"%s\" ucode=%d%s\n",
            matchingInstance->name, int(matchingInstance->ucode),
            matchingInstance->ucode == GBIUCode::Unknown ? "  <<< UNKNOWN, only RDP commands installed" : "");
        fflush(stderr);
