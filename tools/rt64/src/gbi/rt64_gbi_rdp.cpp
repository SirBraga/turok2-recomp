    ...
    namespace GBI_RDP {
        ...
        void texrect(State *state, DisplayList **dl) {
            ...
            *dl = *dl + 1;
            ...
            // TEMPORARY DIAGNOSTIC: Turok 2 draws its opening screen entirely with
            // textured rectangles rather than triangles, so this is what proves
            // the renderer is receiving actual image content.
            {
                static unsigned long total = 0;
                static unsigned reports = 0;
                total++;
                if ((total % 500) == 0 && reports++ < 8) {
                    fprintf(stderr, "[rt64:texrect] %lu drawn, latest rect (%d,%d)-(%d,%d) tile %u\n",
                        total, ulx >> 2, uly >> 2, lrx >> 2, lry >> 2, tile);
                    fflush(stderr);
                }
            }
