...
namespace RT64 {
    namespace GBI_F3DEX2 {
        ...
        // TEMPORARY DIAGNOSTIC: proves geometry is reaching the renderer. Before
        // the Acclaim microcode was mapped to F3DEX2 these handlers were never
        // installed at all, so a non-zero count here means display lists are now
        // being interpreted correctly.
        void countTriangles(unsigned n) {
            static unsigned long total = 0;
            static unsigned reports = 0;
            total += n;
            if ((total % 20000) < n && reports++ < 12) {
                fprintf(stderr, "[rt64:geo] triangles drawn so far: %lu\n", total);
                fflush(stderr);
            }
        }
