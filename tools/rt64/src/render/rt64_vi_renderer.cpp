...
namespace RT64 {
    ...
    void VIRenderer::getViewportAndScissor(const RenderSwapChain *swapChain, const VI &vi, hlslpp::float2 resolutionScale, uint32_t downsamplingScale, bool removeBlackBorders, RenderViewport &viewport, RenderRect &scissor) {
        ...
        // specified by the VI.
        const hlslpp::float2 viFbSize = hlslpp::float2(vi.fbSize());
        const hlslpp::float2 sdSize = (viFbSize.y > 0.0f) ? viFbSize : (removeBlackBorders ? viFbSize : hlslpp::float2(480.0f, 360.0f));
        ...
        static uint64_t viewportDiagnosticCounter = 0;
        static const bool viewportDiagnostics = std::getenv("TUROK2_VIEWPORT_DIAGNOSTICS") != nullptr;
        viewportDiagnosticCounter++;
        if (viewportDiagnostics && ((viewportDiagnosticCounter % 120) == 0)) {
            std::fprintf(stderr,
                "[rt64:viewport] frame=%llu window=%.0fx%.0f fb=%ux%u sd=%.1fx%.1f scale=%.4fx%.4f hd=%.1fx%.1f "
                "view=%.4f,%.4f,%.4f,%.4f crop=%.4f,%.4f,%.4f,%.4f viewport=%.1f,%.1f,%.1f,%.1f scissor=%d,%d,%d,%d remove_borders=%d\n",
                static_cast<unsigned long long>(viewportDiagnosticCounter), float(windowSize.x), float(windowSize.y),
                uint32_t(vi.fbSize().x), uint32_t(vi.fbSize().y), float(sdSize.x), float(sdSize.y),
                float(resolutionScale.x), float(resolutionScale.y), float(hdSize.x), float(hdSize.y),
                float(viViewRect.x), float(viViewRect.y), float(viViewRect.z), float(viViewRect.w),
                float(viCropRect.x), float(viCropRect.y), float(viCropRect.z), float(viCropRect.w),
                viewport.x, viewport.y, viewport.width, viewport.height,
                scissor.left, scissor.top, scissor.right, scissor.bottom, removeBlackBorders ? 1 : 0);
        }
    }
};
