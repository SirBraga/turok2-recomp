...
namespace RT64 {
    ...
    bool RenderTarget::resize(RenderWorker *worker, uint32_t newWidth, uint32_t newHeight) {
        // Keep at least one pixel in each direction. The VI registers read as zero
        // for a frame or two while a game changes video mode, and the present path
        // turns those into a 0 by 0 request; no backend can make a texture that
        // size.
        newWidth = std::max(newWidth, 1u);
        newHeight = std::max(newHeight, 1u);

        // A target that has no texture yet has to be set up even when the size
        // asked for is not larger than the one recorded, which is exactly the
        // state a freshly created target is in, since both are zero. Without
        // this, a new target asked for 0 by 0 kept a null texture and
        // clearColorTarget went on to hand a null color attachment to the
        // backend, which crashed inside MetalFramebuffer's constructor. The
        // assert that would have caught it there is compiled out in release.
        const bool insufficientSize = (width < newWidth) || (height < newHeight);
        if (insufficientSize || (texture == nullptr)) {
