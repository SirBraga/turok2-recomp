...
#   endif
        ...
        static const bool widescreenDiagnostics = std::getenv("TUROK2_WIDESCREEN_DIAGNOSTICS") != nullptr;
        static uint64_t widescreenEdgeReports = 0;
        static uint64_t widescreenProjectionReports = 0;
        auto reportEdgeRect = [&](const char *kind, const GameCall &call) {
            if (!widescreenDiagnostics || (p.aspectRatioTarget <= (p.aspectRatioSource + 0.001f)) || (widescreenEdgeReports >= 240)) {
                return;
            }

            const FixedRect &rect = call.callDesc.rect;
            const FixedRect &callScissor = call.callDesc.scissorRect;
            const int32_t width = rect.width(false, true);
            const bool narrow = width <= 4;
            const bool atLeftEdge = rect.left(false) <= (fbPair.scissorRect.left(false) + 4);
            const bool atRightEdge = rect.right(true) >= (fbPair.scissorRect.right(true) - 4);
            if (narrow && (atLeftEdge || atRightEdge)) {
                widescreenEdgeReports++;
                std::fprintf(stderr,
                    "[rt64:wide-edge] #%llu kind=%s call=%u cycle=%u rect=%d,%d,%d,%d px=%d,%d,%d,%d "
                    "scissor=%d,%d,%d,%d pair=%d,%d,%d,%d fill=%08X ratio=%.4f\n",
                    static_cast<unsigned long long>(widescreenEdgeReports), kind, call.callDesc.callIndex,
                    call.callDesc.otherMode.cycleType(), rect.ulx, rect.uly, rect.lrx, rect.lry,
                    rect.left(false), rect.top(false), rect.right(true), rect.bottom(true),
                    callScissor.left(false), callScissor.top(false), callScissor.right(true), callScissor.bottom(true),
                    fbPair.scissorRect.left(false), fbPair.scissorRect.top(false),
                    fbPair.scissorRect.right(true), fbPair.scissorRect.bottom(true),
                    call.callDesc.fillColor, aspectRatioScale);
            }
        };
...
#       endif
            ...
            if (proj.usesViewport()) {
                ...
                if (widescreenDiagnostics && (p.aspectRatioTarget > (p.aspectRatioSource + 0.001f)) && (widescreenProjectionReports < 400)) {
                    widescreenProjectionReports++;
                    const FixedRect viewportRect = viewport.rect(viewportClipRatios);
                    std::fprintf(stderr,
                        "[rt64:wide-proj] #%llu pair=%d,%d,%d,%d proj=%u calls=%u scissor=%d,%d,%d,%d "
                        "viewport=%d,%d,%d,%d intersection=%d,%d,%d,%d wide=%d origin=%u ratio=%.4f\n",
                        static_cast<unsigned long long>(widescreenProjectionReports),
                        fbPair.scissorRect.left(false), fbPair.scissorRect.top(false),
                        fbPair.scissorRect.right(true), fbPair.scissorRect.bottom(true),
                        static_cast<unsigned>(proj.type), proj.gameCallCount,
                        proj.scissorRect.left(false), proj.scissorRect.top(false),
                        proj.scissorRect.right(true), proj.scissorRect.bottom(true),
                        viewportRect.left(false), viewportRect.top(false),
                        viewportRect.right(true), viewportRect.bottom(true),
                        intersectionRect.left(false), intersectionRect.top(false),
                        intersectionRect.right(true), intersectionRect.bottom(true),
                        useWideViewport ? 1 : 0, static_cast<unsigned>(viewportOrigin), aspectRatioScale);
                }
                if (useWideViewport) {
            ...
            for (uint32_t d = 0; (d < proj.gameCallCount) && (globalCallIndex < p.maxGameCall); d++) {
                ...
                if (cycleType == G_CYC_FILL) {
                    ...
                    const int32_t rectWidth = call.callDesc.rect.width(false, true);
                    const bool narrowEdgeGuard = (rectWidth <= 4) &&
                        ((call.callDesc.rect.ulx <= fbPair.scissorRect.ulx) ||
                         (call.callDesc.rect.lrx >= fbPair.scissorRect.lrx));

                    // A rect that spans the whole width of the scissor.
                    // Turok 2 also emits one-pixel guard strips at both 4:3
                    // framebuffer edges. In Expand mode those strips must move
                    // to the physical widescreen edges; keeping the generic
                    // centered-rectangle scale draws visible vertical seams at
                    // the former 4:3 boundary.
                    if (((call.callDesc.rect.ulx <= fbPair.scissorRect.ulx) && (call.callDesc.rect.lrx >= fbPair.scissorRect.lrx)) || narrowEdgeGuard) {
                        invRatioScale = 1.0f;
                    }
...
#               endif
                    {
                        ...
                        case Projection::Type::Rectangle: {
                            ...
                            const int32_t rectWidth = call.callDesc.rect.width(false, true);
                            const bool narrowEdgeGuard = regularOrigins && (rectWidth <= 4) &&
                                ((call.callDesc.rect.ulx <= fbPair.scissorRect.ulx) ||
                                 (call.callDesc.rect.lrx >= fbPair.scissorRect.lrx));
                            if ((tileCopiesUsed || coversScissorWidth || narrowEdgeGuard || call.callDesc.rectAspect == G_EX_ASPECT_STRETCH) && (call.callDesc.rectAspect != G_EX_ASPECT_ADJUST)) {
                                invRatioScale = 1.0f;
                            }
                            else {
