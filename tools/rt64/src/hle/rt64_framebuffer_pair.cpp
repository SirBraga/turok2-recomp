...
namespace RT64 {
    ...
    bool FramebufferPair::hasSceneProjection() const {
        for (uint32_t p = 0; p < projectionCount; p++) {
            const Projection::Type type = projections[p].type;
            if ((type == Projection::Type::Perspective) || (type == Projection::Type::Triangle)) {
                return true;
            }
        }
    ...
    bool FramebufferPair::isFullScreenFill() const {
        // A camera-cut clear is one G_CYC_FILL covering the VI. Cinema
        // letterbox is two thin bars (gameCallCount >= 2 once merged).
        if (!fillRectOnly || (gameCallCount != 1) || drawColorRect.isEmpty()) {
            return false;
        }

        const int32_t fillWidth = drawColorRect.width(false, true);
        const int32_t fillHeight = drawColorRect.height(false, true);
        return (fillWidth >= (int32_t(colorImage.width) - 4)) && (fillHeight >= 200);
    }

    bool FramebufferPair::isPresentableScene() const {
        return earlyPresentCandidate() && !fillRectOnly && !isFullScreenFill();
    }
};