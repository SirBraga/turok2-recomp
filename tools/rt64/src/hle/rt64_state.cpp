        ...
        if (drawStatus.isChanged(DrawAttribute::Texture) || textureCheck) {
            ...
            if (drawCall.tileCount > 0) {
                ...
                for (uint32_t t = 0; t < drawCall.tileCount; t++) {
                    ...
                    // Taken by value so a zero line stride can be substituted
                    // below without the rest of this body seeing an inconsistent
                    // mix of the original and corrected descriptor.
                    auto tile = rdp->tiles[tileIndex];
                    ...
                    if (tile.line == 0) {
                        ...
                        // Dropping the tile outright makes its texel read as zero,
                        // which is not harmless when a combiner uses that texel as
                        // an interpolation factor rather than as a color.
                        //
                        // Turok 2 draws every text glyph this way. Measured on the
                        // menu screens, in 2-cycle mode with LOD off:
                        //   rgb   = (ENVIRONMENT - PRIMITIVE) * TEXEL1 + PRIMITIVE
                        //   alpha = TEXEL0_ALPHA * ENV_ALPHA
                        // TEXEL0 is the glyph cell (I4b, 16x8, tmem 0x100) and
                        // TEXEL1 is the ramp between the engine's two font colors
                        // (COnScreen__SetFontColor's top and bottom color, which
                        // arrive as the primitive and environment colors). The ramp
                        // tile is declared with line = 0, so invalidating it forces
                        // TEXEL1 to zero and collapses the whole expression to
                        // PRIMITIVE, the dark end of the gradient. Every two-color
                        // glyph then renders flat dark, while glyphs whose two
                        // colors are equal are unaffected.
                        //
                        // What the hardware does here is not undefined. The TMEM
                        // address of a texel is base + t * line + s * bpp, so a
                        // line of zero makes the address independent of t: every
                        // row of the tile reads the same words, giving a texture
                        // that varies along S and is constant along T.
                        //
                        // "row0" reproduces that by giving the tile the smallest
                        // stride covering its S range, collapsing its T range to a
                        // single row and clamping T onto it. "widen" instead treats
                        // the stride as one row per line, which fabricates rows the
                        // hardware would never address but does read real loaded
                        // data. "drop" restores the original behavior of discarding
                        // the tile, which forces its texel to zero.
                        //
                        // The three are kept selectable because they are
                        // distinguishable by eye on Turok 2's text, and which one
                        // looks correct is itself diagnostic: if "widen" looks
                        // better than "row0", the game most likely intends a real
                        // two dimensional ramp and the zero line stride is a
                        // symptom of something lost upstream rather than the
                        // game's intent.
                        enum class Line0Mode { Row0, Widen, Drop };
                        static const Line0Mode line0Mode = []() {
                            const char *env = getenv("RT64_LINE0_MODE");
                            if (env != nullptr) {
                                if (strcmp(env, "widen") == 0) {
                                    return Line0Mode::Widen;
                                }
                                if (strcmp(env, "drop") == 0) {
                                    return Line0Mode::Drop;
                                }
                            }
                            return Line0Mode::Row0;
                        }();

                        if (line0Mode == Line0Mode::Drop) {
                            dstCallTile.valid = false;
                            continue;
                        }

                        tile.line = 1;
                        if (line0Mode == Line0Mode::Row0) {
                            tile.lrt = tile.ult;
                            tile.maskt = 0;
                        }

                        // TEMPORARY DIAGNOSTIC: confirm the text draws really do
                        // reach this path, rather than assuming they do.
                        {
                            static uint64_t rescued = 0;
                            static uint32_t reported = 0;
                            rescued++;
                            if (((rescued <= 4) || ((rescued % 5000) == 0)) && (reported++ < 20)) {
                                fprintf(stderr, "[rt64:tile] line=0 tile %u kept (texel%u, fmt=%u siz=%u tmem=%03X, %ux%u), %llu so far\n",
                                    tileIndex, t, tile.fmt, tile.siz, tile.tmem,
                                    (tile.lrs - tile.uls + 4) / 4, (tile.lrt - tile.ult + 4) / 4,
        ...
        if (presentEarly && (workload.fbPairCount > 0)) {
            ...
            for (int32_t f = workload.fbPairCount - 1; (f >= 0) && !viPresented; f--) {
                ...
                for (size_t h = 0; (h < viHistory.history.size()) && !viPresented; h++) {
                    ...
                    const uint32_t viRowBytes = (uint32_t(fbSize.x) * (4U << fbSiz) + 7U) / 8U;
                    const bool addressMatches = (colorImg.address == fbAddress) || (colorImg.address == fbAddress + viRowBytes);
                    if (addressMatches &&
    ...
    void State::updateScreen(const VI &newVI, bool fromEarlyPresent) {
        ...
        if (newVI.visible()) {
            ...
            if (screenFbSiz >= G_IM_SIZ_16b) {
                ...
                // The VI can only address RDRAM, which is 8 MB even with the
                // Expansion Pak, but nothing here checked that. Its registers read
                // as garbage for a frame or two while a game changes video mode,
                // and this copy trusted them: an origin or a size past the end
                // walked off the end of the allocation and killed the process.
                constexpr uint32_t RdramBytes = 8 * 1024 * 1024;
                if (screenFbAddress >= RdramBytes) {
                    screenFbBytes = 0;
                }
                else if (screenFbBytes > (RdramBytes - screenFbAddress)) {
                    screenFbBytes = RdramBytes - screenFbAddress;
                }
