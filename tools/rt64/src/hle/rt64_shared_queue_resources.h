...
namespace RT64 {
    ...
    struct SharedQueueResources {
        ...
        // Diagnostic only, read by TUROK2_PRESENT_TRACE. Both of the above are
        // rebuilt per workload; these persist per color address so a trace can
        // say what the last workload to touch that address actually drew.
        // fillOnly means every call was a G_CYC_FILL rectangle; noScene means no
        // Perspective or Triangle projection at all. Neither gates presentation:
        // 2D frontend screens land in noScene legitimately.
        std::unordered_set<uint32_t> fillOnlyColorAddresses;
        std::unordered_set<uint32_t> noSceneColorAddresses;
        // With three rotating CFBs and one workload per frame, a buffer is
        // rewritten every third workload, which is exactly when VI scans it out.
        // The sets above therefore describe the render in flight, not the image
        // on screen. These hold the state from the previous time each address
        // was written, which is what the present is actually showing.
        std::unordered_set<uint32_t> fillOnlyColorAddressesPrev;
        std::unordered_set<uint32_t> noSceneColorAddressesPrev;
