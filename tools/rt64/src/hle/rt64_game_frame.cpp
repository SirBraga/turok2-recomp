...
namespace RT64 {
    ...
    void GameFrame::match(RenderWorker *worker, WorkloadQueue &workloadQueue, const GameFrame &prevFrame, BufferUploader *velocityUploader, bool &velocityUploaderUsed, bool &tileInterpolationUsed, bool &lookAtInterpolationUsed) {
        ...
        auto matchScenes = [&](const std::vector<GameScene> &curScenes, const std::vector<GameScene> &prevScenes) {
            ...
            static const bool sceneMatchDiagnostics = std::getenv("TUROK2_INTERPOLATION_DIAGNOSTICS") != nullptr;
            static uint64_t sceneMatchDiagnosticCounter = 0;
            static uint64_t sceneCutDiagnosticCounter = 0;
            // Turok 2 changes cameras and projection parameters abruptly during
            // its logo and Iguana sequences. Matching those unrelated scenes
            // makes the synthetic frames sweep or zoom between two authored
            // cuts. Its fastest continuous cinematic motion measured below 40
            // aggregate matrix units, while actual cuts measured 3159-4757.
            // Keep a wide safety margin so legitimate motion is still fully
            // interpolated and only unmistakable scene changes are rejected.
            constexpr float SceneMatchDifferenceLimit = 512.0f;
            for (const MatchCandidate &candidate : matchCandidates) {
                ...
                if (candidate.difference > SceneMatchDifferenceLimit) {
                    if (sceneMatchDiagnostics && (sceneCutDiagnosticCounter < 240)) {
                        sceneCutDiagnosticCounter++;
                        std::fprintf(stderr,
                            "[rt64:scene-cut] #%llu current=%u previous=%u difference=%.6f limit=%.0f\n",
                            static_cast<unsigned long long>(sceneCutDiagnosticCounter),
                            candidate.curIndex, candidate.prevIndex, candidate.difference,
                            SceneMatchDifferenceLimit);
                    }

                    continue;
                }

                if (sceneMatchDiagnostics && (sceneMatchDiagnosticCounter < 600)) {
                    sceneMatchDiagnosticCounter++;
                    std::fprintf(stderr,
                        "[rt64:scene-match] #%llu current=%u previous=%u difference=%.6f\n",
                        static_cast<unsigned long long>(sceneMatchDiagnosticCounter),
                        candidate.curIndex, candidate.prevIndex, candidate.difference);
                }
