...
namespace RT64 {
    ...
    FramebufferChange &FramebufferChangePool::use(RenderWorker *renderWorker, FramebufferChange::Type type, uint32_t width, uint32_t height, bool usesHDR) {
        ...
        // Rounding up leaves zero as zero, and a zero sized texture is something
        // the backend refuses: Metal fails an assertion with "MTLTextureDescriptor
        // has width of zero". A request of that shape does arrive, because the VI
        // registers read as zero for a frame or two while a game changes video
        // mode, and the present path forwards the framebuffer size unchecked. Keep
        // one alignment block so the request stays valid; the change then covers no
        // rows and does nothing, which is the intent.
        if (alignedWidth == 0) {
            alignedWidth = Alignment;
        }

        if (alignedHeight == 0) {
            alignedHeight = Alignment;
        }
