...
> **CORRECTION (2026-08-31).** The original version of this document concluded
> that the graphics ucode belonged to the **F3DEX 1.x** family and was
> **byte-identical stock SDK**, and that therefore "no new handler needs to be
> written". Both conclusions were wrong, and the second one cost real debugging
> time. See "Correction: GBI generation" below for the evidence. The sections
> that follow have been amended.

**Nintendo SDK microcode, F3DEX2 generation, rebuilt by Acclaim (not
byte-identical to the stock SDK blob).** No custom RSP opcodes. RT64 renders it
with its existing `F3DEX2` command table, but the ucode hash had to be added to
RT64's GBI database first — see the implications section.

| Component | Variant | Source | Custom? |
|---|---|---|---|
| Graphics ucode | `gspF3DEX2.NoN.fifo 2.05` — ROM signature spells it `F3DEX.NoN fifo 2.05` (Yoshitaka Yasumoto, 1998 Nintendo) | libultra | Stock *design*, Acclaim *build* — hash differs from stock 2.05 |
| Line ucode | `gspL3DEX2.fifo 2.05` — ROM signature spells it `L3DEX fifo 2.05` | libultra | Same: stock design, Acclaim build |
| Audio ucode | `aspMain` (basic audio, stock) | libultra | NO |
| RSP bootloader | `rspboot` | libultra | NO |

## Correction: GBI generation

The version number, not the name, identifies the GBI generation. The SDK spells
the **F3DEX2** family's signature string as plain `F3DEX` with a **2.xx**
version; the **1.xx** versions are the older F3DEX generation. So
`F3DEX.NoN fifo 2.05` is F3DEX2, and its command numbering is F3DEX2's
(`G_DL` = `0xDE`, `G_ENDDL` = `0xDF`, `G_VTX` = `0x01`), **not** F3DEX 1.x's
(`G_DL` = `0x06`, `G_ENDDL` = `0xB8`, `G_MTX` = `0x01`).

Two independent confirmations, both reproducible locally:

1. **The companion ucode.** This ROM also ships `RSP Gfx ucode L3DEX fifo 2.05`
   (ROM `0xC0898`). RT64's own GBI database labels that exact blob
   `"L3DEX2.fifo 2.05"`, while its `L3DEX 1.00 / 1.21 / 1.23` entries are the
   1.xx generation. RT64's label therefore carries the `2` that the SDK
   signature string omits — which is precisely the naming rule above.
2. **Hash window sizes.** In RT64's database the F3DEX 1.x `.NoN` entries hash
   `0x1430` bytes of text against `0x800` bytes of data, whereas the
   `F3DEX2.NoN.fifo` entries use `0x1390` / `0x420`. This ROM's ucode matches in
   the `0x1390` / `0x420` buckets.

Getting this backwards is not a cosmetic error: decoding an F3DEX2 list with
the F3DEX 1.x table reads every `G_DL` (`0xDE`) as an unknown command and never
finds a terminator, which makes a perfectly healthy display list look empty of
geometry.
...
- **No new command handler is needed, but the ucode had to be registered.**
  RT64 identifies a ucode by hashing its text and data segments against a
  database of known builds. Acclaim's rebuild of `F3DEX2.NoN.fifo 2.05` was not
  in that database, so RT64 fell through to an entry whose ucode was
  `GBIUCode::Unknown`. That path installs **only the RDP command subset** and
  leaves every display list command unhandled — and because the `assert(false)`
  guarding it is compiled out in optimized builds, it fails silently and
  produces nonsensical tile parameters downstream rather than an error.
  Two entries were added to `tools/rt64/src/gbi/rt64_gbi.cpp`:
  `F3DEX2.NoN.fifo 2.05 (Acclaim)` → `GBIUCode::F3DEX2` and
  `L3DEX2.fifo 2.05 (Acclaim)` → `GBIUCode::L3DEX2`.
  The recorded hashes came from an instrumented run, not from an offline
  computation, and are marked "Needs confirmation" in the database accordingly.
- Because those hashes differ from RT64's stock `F3DEX2.NoN.fifo 2.05` entry,
  the claim that this ROM ships a byte-identical stock blob does not hold.
